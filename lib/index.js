...
function supports_brotli(req) {
	const headerValue = req.acceptsEncodings('br');
	return headerValue;
}
...
function get_route_handler(fn) {
	return async function handle_route(req, res, next) {
		...
		const client_files = supports_brotli(req) ? client.brotli : client.default;
			...
			for (const route of route_manager.routes) {
				if (route.test(url)) {
					...
					if (route.type === 'page') {
						...
						res.set('Link', `<${client_files.main_file}>;rel="preload";as="script", <${client_files.routes[route.id]}>;rel="preload";as="script"`);
