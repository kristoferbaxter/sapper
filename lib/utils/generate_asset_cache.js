...
function brotli_path(path) {
	return path.replace('.js', '.br.js');
}
...
module.exports = function generate_asset_cache(clientInfo, serverInfo) {
	...
	const routes = route_manager.routes.reduce((lookup, route) => {
	...
	return {
		client: {
			...
			brotli: {
				main_file: brotli_path(main_file),
				chunk_files: chunk_files.map(chunk => brotli_path(chunk)),
				routes: routes.map(route => brotli_path(route)),
			},
